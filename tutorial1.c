...
//for ADC conversion
uint32_t adcValue; //for single conversion
uint32_t adccValue;//for continuous conversion
/* USER CODE END PTD */
...
//define for led
#define GreenLed GPIO_PIN_6
#define OrangeLed GPIO_PIN_4
#define RedLed GPIO_PIN_5
#define BlueLed GPIO_PIN_3
...
//For Toggle of any leds
#define ToggleLeds(led) \
do{ \
 if ((led)==GreenLed){ \
	 HAL_GPIO_TogglePin(GPIOG, GreenLed); \
 }else if ((led)==OrangeLed){ \
...
//May be the led are on by default So for off i am setting it and for on i am resetting
#define LedOff(led) \
do{ \
 if ((led)==GreenLed){ \
	 HAL_GPIO_WritePin(GPIOG, GreenLed, GPIO_PIN_SET); \
 }else if ((led)==OrangeLed){ \
...
do{ \
 if ((led)==GreenLed){ \
	 HAL_GPIO_WritePin(GPIOG, GreenLed, GPIO_PIN_RESET); \
 }else if ((led)==OrangeLed){ \
...
void singconvADC(){
	//ADC single conversion mode
		  //Read ADC value
		  HAL_ADC_Start(&hadc1);
		  //check for adc conversion if HAL_Ok then getvalue
		  if(HAL_ADC_PollForConversion(&hadc1, 5)==HAL_OK){
			  adcValue=HAL_ADC_GetValue(&hadc1);
		  }
		  //clear all the ADC flag
		  HAL_ADC_Stop(&hadc1);
		  HAL_Delay(100);
}
...
void contconvADC(){
	//ADC single conversion mode
		  ...
		  //then getvalue
		  adccValue=HAL_ADC_GetValue(&hadc2);
...
/* Configure the peripherals common clocks */
  ...
  while (1)
  {
	  ...
	  //function for turning led on or off
	  LedOff(GreenLed);
	  LedOn(OrangeLed);

	  //read the push button state
	  myPushButton=HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_0);
	  HAL_GPIO_WritePin(GPIOK,BlueLed,!myPushButton);
	  HAL_Delay(100);
